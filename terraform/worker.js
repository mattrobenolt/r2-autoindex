// Code generated by `npm run build:standalone`; DO NOT EDIT.
  ...
  "node_modules/mime-types/index.js"(exports) {
    ...
    function populateMaps(extensions, types) {
      Object.keys(db).forEach(
        /* @__PURE__ */ __name(function forEachMimeType(type) {
          var mime = db[type];
...
var require_dist = __commonJS({
  "node_modules/path-to-regexp/dist/index.js"(exports) {
    ...
    function pathToRegexp(path, options = {}) {
      ...
      function process(path2) {
        if (Array.isArray(path2)) {
          for (const p of path2) process(p);
          return;
        }
    ...
    function stringifyName(name, next) {
      ...
      if (
        (next === null || next === void 0 ? void 0 : next.type) === "text" &&
        ID_CONTINUE.test(next.value[0])
      ) {
...
function createAutoIndexWorker(config = {}) {
  ...
  __name(routeConfig, "routeConfig");
  return {
...
async function servePath(request, bucket, internalPrefix, routes, path) {
  if (isInternalKey(path.key, internalPrefix) || matchesAny(routes.denied, path.pathname)) {
    return new Response("Not Found", { status: 404 });
  }
  if (!path.directory) {
    const range = parseRange(request.headers.get("range"));
    const object = await bucket.get(path.key, range ? { range } : void 0);
...
function isInternalKey(key, internalPrefix) {
  return key === internalPrefix.slice(0, -1) || key.startsWith(internalPrefix);
}
__name(isInternalKey, "isInternalKey");
async function serveDirectory(bucket, prefix, internalPrefix, requestPath, method, routes) {
...
__name(serveDirectory, "serveDirectory");
async function listDirectory(bucket, prefix, internalPrefix, routes) {
  const entries = [];
  let cursor;
  do {
...
__name(listDirectory, "listDirectory");
async function directoryHasEntries(bucket, prefix, internalPrefix, routes) {
...
__name(directoryHasEntries, "directoryHasEntries");
function renderAutoindex(pathname, entries) {
...
__name(renderAutoindex, "renderAutoindex");
...
function parseRange(header) {
  ...
  if (last < offset) return void 0;
  return { offset, length: last - offset + 1 };
}
__name(parseRange, "parseRange");
function objectResponse(object, method, requestedRange) {
  const headers = new Headers();
  object.writeHttpMetadata(headers);
  const inferredType = (0, import_mime_types.lookup)(object.key);
  if (
    inferredType &&
...
function resolvedRange(range, size) {
  if ("suffix" in range) {
    const length2 = Math.min(range.suffix, size);
    return { offset: size - length2, length: length2 };
  }
...
__name(resolvedRange, "resolvedRange");
function applyErrorPage(response, method, errors) {
...
__name(applyErrorPage, "applyErrorPage");
function finalizeResponse(response, method, routes, pathname, auth) {
  ...
  return auth?.cache === "allow" ? withHeaders : applyNoStore(withHeaders, auth !== void 0);
}
__name(finalizeResponse, "finalizeResponse");
function applyHeaders(response, rules, pathname) {
  const headers = new Headers(response.headers);
...
function applyNoStore(response, enabled) {
...
async function buildRouteConfig(config) {
...
__name(buildRouteConfig, "buildRouteConfig");
async function authenticate(request, env, rule) {
...
function basicCredentials(header) {
  if (!header?.startsWith("Basic ")) return void 0;
  try {
...
__name(basicCredentials, "basicCredentials");
...
function getBucket(env, config) {
  if (typeof config.bucket === "function") return config.bucket(env);
  const binding = config.bucket ?? "BUCKET";
...
__name(normalizeRewrites, "normalizeRewrites");
function compilePatternList(patterns) {
  return compileRules(patterns.map((source) => ({ source })));
}
__name(compilePatternList, "compilePatternList");
...
function compileMatcher(source) {
  ...
  return (0, import_path_to_regexp.match)(source, { decode: decodeURIComponent });
}
__name(compileMatcher, "compileMatcher");
...
function matchesAny(rules, pathname) {
  return firstMatch(rules, pathname) !== void 0;
}
__name(matchesAny, "matchesAny");
function rewritePath(path, destination, params) {
...
function applyDestination(destination, params) {
  ...
  if (/^[a-z][a-z0-9+.-]*:/i.test(destination)) {
    ...
    url.pathname = (0, import_path_to_regexp.compile)(url.pathname, { encode: encodeURIComponent })(
      params,
    );
    return url.toString();
  }
  return (0, import_path_to_regexp.compile)(destination, { encode: encodeURIComponent })(params);
}
__name(applyDestination, "applyDestination");
...
function ancestorPrefixes(key) {
  const prefixes = [""];
  const parts = key.split("/").filter(Boolean);
  for (let i = 1; i < parts.length; i++) prefixes.push(`${parts.slice(0, i).join("/")}/`);
  return prefixes;
}
__name(ancestorPrefixes, "ancestorPrefixes");
async function deleteInChunks(bucket, keys) {
  for (let i = 0; i < keys.length; i += 1e3) await bucket.delete(keys.slice(i, i + 1e3));
}
__name(deleteInChunks, "deleteInChunks");
function indexKey(prefix, internalPrefix) {
  return prefix === ""
...
function parseConfig(input) {
  if (input === void 0) return DEFAULT_CONFIG;
...
function toAutoIndexConfig(config) {
  return {
    ...
    auth: config.auth?.map((rule) => ({
      ...
      realm: rule.realm,
      cache: rule.cache,
      verify: /* @__PURE__ */ __name(async ({ username, password }) => {
        const user = rule.users.find((candidate) => candidate.username === username);
        return user !== void 0 && (await sha256Hex(password)) === user.passwordSha256;
      }, "verify"),
    })),
    headers: config.headers ? async () => config.headers ?? [] : void 0,
    redirects: config.redirects ? async () => config.redirects ?? [] : void 0,
